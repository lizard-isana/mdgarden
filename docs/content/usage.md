...
```HTML
<link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/lizard-isana/mdgarden@v0.3.0/dist/css/default.css">
<script src="https://cdn.jsdelivr.net/gh/lizard-isana/mdgarden@v0.3.0/dist/js/mdgarden.min.js"></script>
```
※ 上記の例では、常に最新版を読み込みます。互換性などの問題から特定のバージョンが必要な場合は、下記からリンクを取得してください。
cf. https://www.jsdelivr.com/package/gh/lizard-isana/mdgarden
...
2. `&lt;md-garden&gt;`のタグを書く(任意の`id`を指定する)
...
1. スクリプトとスタイルシートを読みこむ
2. `&lt;md-garden&gt;`のタグを書く(任意の`id`と外部Markdownファイルを指定する)

```HTML
<md-garden id="main" src="main.md">
...
```HTML
<md-garden id="main" src="main.md" data-spa="true">
...
MDGardenはデフォルトの状態では、DOMPurifyを使って、生成されるHTML上でスクリプトが動作するのを抑制しています。そのため、スクリプトタグを始めとして、一部のタグが動作しません。以下のように、optionで `data-html="true"`を指定すると本文中でHTMLタグが使えるようになり、さらに `data-sanitize="false"`を指定するとスクリプトが使えるようになります。

```HTML
<md-garden id="main" data-html="true" data-sanitize="false"></md-garden>
...
YAML形式のフロントマター(Front Matter)に対応しています。Markdownファイルの冒頭に下記のようにYAML形式でメタ情報を書き込んでおくと、JavaScriptから`MDGarden[id].meta`でアクセスすることができます。
...
```JavaScript
/* <md-garden id="main"></md-garden> 内に読み込んだ場合を想定 */
MDGarden.main.meta = {
  "title":"Page Title",
  "date":"2023-07-09 12:00",
  "update": "2023-07-10 12:00",
...
```JavaScript
const target = document.querySelector("md-garden#main");
const observer = new MutationObserver( (mutations) => {
  const content = mutations[0].target;
  if(MDGarden.main.meta.title){
      document.title = `${MDGarden.main.meta.title}`
  }else{
...
## 組み込みプラグイン
以下の機能は組み込みプラグインとして提供されています。`&lt;md-garden&gt;`内にプラグインコンポーネントを追記することでアクティブになります。
...
以下のように、`data-plugins`属性を記述することで有効化できます
...
目次は`data-plugins`属性で指定しなくてもデフォルトで有効になっています。
...
コードハイライトは`data-plugins`属性で指定しなくてもデフォルトで有効になっています。
cf.[highlight.js](https://highlightjs.org/)

````
```HTML
<md-garden id="main"></md-garden>
<template class="markdown" data-target="main">
  # H1
  Markdownをここに直接書く
</template>
...
#### スタイル
以下のように、`data-highlight-style`属性を指定することで、highlight.jsのスタイルを指定できます。何も指定しないと、`github`スタイルが適用されます。

```HTML
<md-garden id="main" data-highlight-style="github"></md-garden>
```
...
## Copyrights/Credits
MDGarden depends on below superb libraries.
