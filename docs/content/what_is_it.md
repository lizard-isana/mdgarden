...
## Document Viewer
MDGardenは、公開サーバでは静的なMarkdownファイルをリッチでインタラクティブなページに変換する高機能なドキュメントビューアーとして動作します。
...
## Wiki Engine
MDGardenは、ローカルサーバ（localhost/127.0.0.1）では簡易編集やインデックス機能を備えたWikiエンジン／パーソナルナレッジベースとして動作します。また、プロジェクト全体をサーバレスで動作するシングルファイルWikiとして書き出すことができます。
