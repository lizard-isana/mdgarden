...
ローカル環境ではFile System Access APIを使って、ブラウザ上からMarkdownを直接編集する機能やプロジェクト全体を1つのHTMLとして書き出す機能を提供します。ブラウザの制限により保存処理はローカルに対してのみ行われ、ブラウザとOS間のネイティブな機能だけで完結します。サーバ上では動的なプロセスは一切走らないため、悪意のある第三者がファイルを改ざんするリスクが極めて小さくなります[^1]。
