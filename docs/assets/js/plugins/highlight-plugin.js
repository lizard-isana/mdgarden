...
const markCodeBlocks = (target) => {
  ...
  for (const codeElement of code_array) {
    ...
    if (class_list && class_list.value.match(/language/)) {
      ...
      codeElement.setAttribute('data-mdgarden-highlight', HIGHLIGHT_EXCEPTIONS.indexOf(lang) < 0);
    }
    if (
      lang &&
      ...
      codeElement.dataset.mdgardenHighlighted !== 'true' &&
    ...
    if (class_list && codeElement.dataset.mdgardenHighlighted === 'true') {
      class_list.add('hljs');
    }
    ...
    if (
      codeElement.dataset &&
      codeElement.dataset.mdgardenHighlight &&
      codeElement.dataset.mdgardenHighlight === 'true' &&
      codeElement.dataset.mdgardenLineNumbered !== 'true' &&
      window.hljs &&
  ...
  return {
    ...
    onInit: async ({ ctx }) => {
      try {
        ...
        console.warn('[MDGarden] Highlight assets are unavailable on init.', error);
      }
      if (!resizeHandler) {
        resizeHandler = () => {
          if (resizeTimer) {
    ...
    onEvent: async ({ event, payload, ctx }) => {
      ...
      try {
        ...
        console.warn('[MDGarden] Highlight assets are unavailable.', error);
      }
      if (!payload) {
        return;
      }
