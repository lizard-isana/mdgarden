...
const createInlineSpaPlugin = () => {
  ...
  const rewriteInlineLinks = (viewer, payloadRoot) => {
    ...
    for (const link of links) {
      ...
      link.dataset.mdgardenInlineSpaPage = page;
      link.dataset.mdgardenInlineSpaParam = queryParam;
      if (typeof viewer.BuildInlineSpaUrl === "function") {
        link.href = viewer.BuildInlineSpaUrl(queryParam, page);
      }

      if (link.dataset.mdgardenInlineSpaBound === "true") {
        continue;
      }
      if (!clickHandler) {
        clickHandler = (event) => {
          ...
          const targetPage = targetLink.dataset.mdgardenInlineSpaPage;
          const param = targetLink.dataset.mdgardenInlineSpaParam || getQueryParam(activeViewer);
          if (!targetPage) {
            return;
          }
      ...
      link.dataset.mdgardenInlineSpaBound = "true";
