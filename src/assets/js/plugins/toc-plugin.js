...
const renderToc = (viewer, root) => {
  ...
  const header_array = container.querySelectorAll('h2, h3');
  const toc_str_array = ['## Contents'];
  ...
  const toc_html = renderer.render(toc_str_array.join('\n'));
  const toc_array = viewer.querySelectorAll('.toc');
  toc_array.forEach((element) => {
    element.innerHTML = toc_html;
    const toc_title = element.querySelector('h2');
    if (!toc_title) {
      return;
    }
