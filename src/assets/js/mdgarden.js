...
import {createMDGardenPluginRuntime} from './plugins/plugin-runtime.js'
...
const GlobalStorage = {}
GlobalStorage.mdgarden = [];
GlobalStorage.Hook = {}

window.MDGarden = window.MDGarden || {
...
const RunCodeHighlightHooks = (id, code, lang, fallback) => {
  ...
  if (window.MDGarden.pluginRuntime) {
    const runtimeTransformed = window.MDGarden.pluginRuntime.transformCode(id, next, lang);
...
const EmitPluginEvent = (viewId, eventName, payload) => {
  ...
  window.MDGarden.pluginRuntime.emit(viewId, eventName, payload);
}

window.MDGarden.PLUGIN_EVENTS = PluginEvents;
window.MDGarden.registerHook = (viewId, eventName, handler) => {
  GlobalAddHook(viewId, eventName, handler);
};
window.MDGarden.loadScripts = LoadScriptsAsync;
window.MDGarden.loadStyles = LoadStylesAsync;
window.MDGarden.pluginRuntime = window.MDGarden.pluginRuntime || createMDGardenPluginRuntime({
  loadScripts: LoadScriptsAsync,
  loadStyles: LoadStylesAsync
});
window.MDGarden.registerPlugin = (viewId, plugin) => {
  return window.MDGarden.pluginRuntime.registerPlugin(viewId, plugin);
};
...
class MarkdownViewer extends HTMLElement {
  constructor() {
    ...
    if(GlobalStorage.mdgarden.indexOf(this.id)<0){
      GlobalStorage.mdgarden.push(this.id);
    }
    window.MDGarden[this.id] = window.MDGarden[this.id] || {}
    ...
    this.BindPublicApi();
    registerBuiltinPlugins(window.MDGarden, this);
    window.MDGarden.pluginRuntime.initViewer(this);
  } 
  LoadOptionConfig = async (url, optional = true) => {
    ...
    const promise = (async () => {
      try {
        ...
        if (!response.ok) {
          if (!optional) {
            console.warn(`[MDGarden] Failed to load config: ${key} (${response.status})`);
          }
          return {};
        }
        const data = await response.json();
        if (!data || typeof data !== "object" || Array.isArray(data)) {
          console.warn(`[MDGarden] Invalid config format: ${key}`);
          return {};
        }
        return data;
      } catch (error) {
        if (!optional) {
          console.warn(`[MDGarden] Failed to parse config: ${key}`, error);
        }
        return {};
      }
    })();
  ...
  ParseInlineOption = () => {
    ...
    try {
      ...
      console.warn("[MDGarden] Invalid data-option JSON", error);
      return {};
    }
  }
  ...
  setMarkdown = (markdown, option = {}) => {
    if (this.getAttribute("src")) {
      console.warn("[MDGarden] setMarkdown is only available in inline mode.");
      return null;
    }
  ...
  setPage = (requestedPage, option = {}) => {
    if (this.getAttribute("src")) {
      console.warn("[MDGarden] setPage is only available in inline mode.");
      return null;
    }
  ...
  BindPublicApi = () => {
    ...
    const api = window.MDGarden[this.id] || {};
    api.element = this;
  ...
  RewriteIncludeLinks = (dom, loadingTarget, baseDocPath = "") => {
    ...
    for (var i = 0, ln = link_array.length; i < ln; i++) {
      ...
      link_array[i].dataset.mdgardenTarget = normalizedTarget;
      link_array[i].href = this.BuildViewerUrl(normalizedTarget);
      if (link_array[i].dataset.mdgardenBound === "true") {
        continue;
      }
      link_array[i].dataset.mdgardenBound = "true";
  ...
  OnMdLinkClick = (e) => {
    e.preventDefault();
    const file = e.currentTarget.dataset.mdgardenTarget;
    if (!file) {
      return;
    }
  ...
  load = async (target, loadOption = {}) => {
    ...
    const mdgarden_content = document.querySelector(`md-garden#${this.id}`);
    ...
    if(src){
      ...
      if(target){
        ...
        if (!normalizedTarget) {
          console.warn(`[MDGarden] Blocked target path: ${target}`);
        ...
        for (var key in this.query) {
          ...
          if (key == this.id) {
            ...
            if (normalizedTarget) {
              ...
              console.warn(`[MDGarden] Blocked query path: ${this.query[key]}`);
              file = src;
            }
          }else{
            file = src;
      ...
      if (!loadResult.ok) {
        if (loadResult.error) {
          console.warn(`[MDGarden] Failed to load markdown: ${loadResult.error}`);
        }
    ...
    this.renderer
      ...
      if(this.option.frontmatter == true){
        this.renderer.use(markdownItMetaYaml, {
          cb: (meta) => {
            if(meta){
              window.MDGarden[this.id].meta = meta;
            }
          }
        })
      }
  ...
  HandlePopState = () => {
    ...
    const query_keys = Object.keys(q).filter(i => GlobalStorage.mdgarden.indexOf(i) >= 0);
...
window.addEventListener('DOMContentLoaded', function() {
  if (!customElements.get('md-garden')) {
    customElements.define('md-garden', MarkdownViewer);
  }
})
